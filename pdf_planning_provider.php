...
/*------------------------------------------------------------PRISE DE RDV---------------------------------------------------------*/
    ...
    foreach ($arrayDateBis as $key => $value) {
        ...
        $testRdv = $appService->findRdvFournisseurByThree($enterpriseConcerned->getIdEnterprise(), $purchasingFairConcerned->getIdPurchasingFair(), $value);
        ...
        if($testRdv != null){
            ...
            $content .= '
                <th style="vertical-align:middle;text-align:center;border:solid black 1px;font-size:6px; width:9%">MAGASIN</th>';
            ...
            $AllRdvStore = $appService->findRdvFournisseurByThree($enterpriseConcerned->getIdEnterprise(), $purchasingFairConcerned->getIdPurchasingFair(), $arrayGlobale[0]["jour"]); //rdv fournisseur du jour selectionné
            ...
            if($AllRdvStore != null){
               ...
               // print_r($DifferentsFournisseurs);
                ...
                foreach ($DifferentsFournisseurs as $key => $DF) {
                    ...
                    $CommercialRDV = $appService->findRdvFournisseurByQuattro($enterpriseConcerned->getIdEnterprise(), $purchasingFairConcerned->getIdPurchasingFair(), $arrayGlobale[0]["jour"], $DF[0]);
                                ...
                                foreach ($HeureIndispo2 as $key => $HeureIndispo2bis) {
                                    ...
                                    foreach ($HeureIndispo2bis as $key => $HeureIndispo2bisbis) {
                                        if($indispo == $HeureIndispo2bisbis){
                                            ...
                                            $ref = "1";
                                            $content .= '<td style="background-color:#2CC36B;border:1px solid black;width:3.4%;font-size:7px; font-weight:bold; vertical-align:middle;">'.$obj3.'</td>';
                                        }
                                    }
                                }
                            }
            ...
            $nbRepas = $appService->findLunchForOneEnterpriseAndPfAndDay2($enterpriseConcerned->getIdEnterprise(), $purchasingFairConcerned->getIdPurchasingFair(), $datedate);

            $RepasExceptionnels = $appService->findOneSpecialGuestByThree($purchasingFairConcerned->getIdPurchasingFair(), $datedate, $enterpriseConcerned->getIdEnterprise());
