...
if( isset($_POST) && !empty($_POST) ) {
    ...
    for($z = 1 ; $z <= 5 ; $z++) {
        ...
        if( isset($postDateTime) && !empty($postDateTime) ) { // Period Datetime values			
            ...
            $idUnavailability  = $appService->saveUnavailabilitySp($appService->createUnavailabilitySp($dataDatetime['startDatetime'], $dataDatetime['endDatetime'], $_SESSION['salespersonsConcerned'][$_SESSION['salespersonsConcernedIterator']], $_SESSION['purchasingFairConcerned']));
        }
    ...
    header('Location: ./admin_sp_unav_register.php'); // Home page redirection
}

$arrayUnavailabilitiesAlreadyRegistered = $appService->findParticipantUnavailabilitiesSp($_SESSION['salespersonsConcerned'][$_SESSION['salespersonsConcernedIterator']],$_SESSION['purchasingFairConcerned']);

//var_dump($_SESSION['salespersonsConcerned']);
//var_dump($_SESSION['salespersonsConcernedIterator']);
?>
...
<body>
            ...
            <div class="row wrapper border-bottom white-bg page-heading">
                <div class="col-lg-9">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">
                            <a href="./store_choice_providers.php">Choix des commerciaux</a>
                        </li>                        
                        <li class="active">
                ...
                <div class="row" >
                    ...
                    if( $limit ) {
                        foreach($arrayUnavailabilitiesAlreadyRegistered as $value) {
                            $counter++;
                            echo '<span id="rowUnavailabilitySp_'.$value->getIdUnavailability().'">';
                            ...
                            echo '&nbsp;|&nbsp;';
                            echo '<i class="fa fa-window-close text-danger" title="Supprimer" aria-hidden="true" onclick="deleteUnavailabilitySp('.$value->getIdUnavailability().');"></i>';                            echo '</span>';
                    ...
                    <div class=" col-lg-offset-3 col-lg-7 text-center alert alert-warning animated fadeInDown">
                    <?php 
                        echo '<i class="fa fa-info-circle" aria-hidden="true"></i> Saisie indisponibilités Commercial N°'.($_SESSION['salespersonsConcernedIterator']+1).'/'.count($_SESSION['salespersonsConcerned']);
                        echo'<br>';
                        echo $_SESSION['salespersonsConcerned'][$_SESSION['salespersonsConcernedIterator']];
                        ?>
                    </div>
    ...
    */
    ...
    $("#previousButton").click(function(){ 
        ...
        <?php if( empty($_SESSION['salespersonsConcernedIterator']) ) { ?>
        window.location.assign("./admin_list_of_stores_by_salesperson.php"); 
        <?php } else { ?>
        $.post(
           './ajax/changeSalespersonsIterator.php',
           {
               action    : 'minus'
           },
           function(data) {
               // // https://www.w3schools.com/jsref/met_loc_reload.asp
               location.reload(true); // Bypass cache - Bug reported by Céline Service Textile - Fixed 04/09/2018
...
//               }
        ...
        // window.location.assign("./admin_sp_unav_register.php"); Use cache - generate errors
    ...
    $("#nextButton").click(function(){ 
        ...
        <?php if( $_SESSION['salespersonsConcernedIterator'] == count($_SESSION['salespersonsConcerned']) - 1 ) { ?>
        window.location.assign("./purchasing_fair_list.php"); 
        <?php } else { ?>
        $.post(
           './ajax/changeSalespersonsIterator.php',
           {
...
//               }
    ...
    function deleteUnavailabilitySp(idUnavailabilitySp) {
        ...
        bootbox.confirm("Confirmer la suppression ?", function(result) { 
            ...
            if(result === true) {
                ...
                $.post(
                    './ajax/deleteUnavailabilitySp.php',
                    {
                        idUnavailabilitySp : idUnavailabilitySp
                    },
                    function(data) {
                        if(data.trim() === 'Success') {
                            $('#rowUnavailabilitySp_' + idUnavailabilitySp).html('<span class="text-danger"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i>&nbsp;Indisponibilité supprimée.</span>');
                            toastr.success('Action réussie', 'Succès.');
                        }
                        else {
                            toastr.error('Erreur fatale. Contactez l\'administrateur.', 'Échec.');
