...
class LogDAO extends AbstractDAO {
    ...
    public function findAll() {
        ...
        try {
            ...
            while( $row = $qresult->fetch() ) {
                    list ( $idLog, $idEnterprise, $ipAddress, $actionDateTime ) = $row; // Like that $idLog = $row['id_log'] etc.
                    $newLog = new Log($idEnterprise, $ipAddress, $actionDateTime, $idLog);
    ...
    public function insert($log) { 
        ...
        $query = "INSERT INTO log (ENTERPRISE_id_enterprise, ip_address, action_datetime) VALUES(?,?,NOW())";

        try {
            $this->pdo->beginTransaction(); // Start transaction
            $qresult = $this->pdo->prepare($query); 
            $qresult->execute(array($log->getOneEnterprise()->getIdEnterprise(), $log->getIpAddress()));
    ...
    public function deletelog($IdEnterprise) {
        $query = "DELETE FROM log WHERE ENTERPRISE_id_enterprise = ?";
