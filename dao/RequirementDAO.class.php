...
class RequirementDAO extends AbstractDAO {
     ...
     * Returns the collection (a simple array) of all requirements (the returned array may be empty)
     */
    public function findAllByPf($idPurchasingFair) {

        $result = array ();
        $query = "SELECT * FROM requirement WHERE PURCHASING_FAIR_id_purchasing_fair = ?";
        ...
        try {
			...
			// https://openclassrooms.com/fr/courses/1262531-pdo-comprendre-et-corriger-les-erreurs-les-plus-frequentes
			// http://se.php.net/manual/en/pdo.prepare.php
			// https://www.w3schools.com/php/php_mysql_prepared_statements.asp
			$qresult->bindValue(1, (int) $idPurchasingFair, PDO::PARAM_INT); // Default value == PARAM_STR, MySQL is permissive
			$qresult->execute();
            $this->pdo->commit(); // If all goes well the transaction is validated
            while( $row = $qresult->fetch() ) {
                    list ( $idRequirement, $idStore, $idProvider, $idPurchasingFair, $numberOfHours ) = $row; // Like that $idPurchasingFair = $row['id_purchasing_fair'] etc.
                    $newRequirement = new Requirement($idStore, $idProvider, $idPurchasingFair, $numberOfHours, $idRequirement);
                    $result[$idRequirement] = $newRequirement; // Adds new PurchasingFair to array
            }
    ...
    /*
     * Returns the objects searched requirements for a duo { store - purchasing fair } SORTED VERSION
     */
    public function findByDuoSorted($oneStore, $onePurchasingFair) {

        $result = array();
        $query = '
            SELECT * 
            FROM requirement req
            INNER JOIN enterprise ent on ent.id_enterprise = req.ENTERPRISE_PROVIDER_id_enterprise
            WHERE req.ENTERPRISE_STORE_id_enterprise = ? AND req.PURCHASING_FAIR_id_purchasing_fair = ?
            ORDER BY ent.name_enterprise
            ';
