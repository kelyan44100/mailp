...
//    var_dump($arrayPP);
    ...
    foreach($arrayPP as $key => $pp) {
        $arraySalespersons[$pp->getOneProvider()->getIdEnterprise()] = $appService->findAllParticipantsAsSalespersonsByProviderAndPf($pp->getOneProvider()->getIdEnterprise(), $_SESSION['purchasingFairConcerned']->getIdPurchasingFair());
        $counterSalespersons += count($arraySalespersons[$pp->getOneProvider()->getIdEnterprise()]);
    }
...
//    die();
    ...
    // Total mails to send
    $counterMailsToSend = $counterSalespersons;

    //print_r($counterMailsToSend);
        
    // Check sending
    $counterSuccesfulSendings = 0;
    
    // Mails for Salesperson concerned
    foreach($arraySalespersons as $provider => $salespersons) {
        
        $totSalespersons = count($salespersons);
        
        for( $j = 0 ; $j < $totSalespersons ; $j++ ) {
        
            $salesperson = $salespersons[$j];

            if( !empty( $salesperson->getEmail() ) ) {
                ...
                $idSp         = $salesperson->getIdParticipant();
                $nameProvider = $appService->findOneEnterprise($provider)->getName();
                $key          = $appService->randomKey();

                $body  = 'Bonjour,<br/><br/>';
                $body .= 'Vous pouvez dès à présent consulter votre planning pour le salon d\'achats mentionné en objet en cliquant sur ce lien : ';
                $body .= '<a href="http://www.scaouest.info/pf_management/pdf_planning_sp.php?k='.$key.'&pf='.$idPf.'&sp='.$idSp.'" target="_blank">LIEN</a><br/><br/>';
                $body .= '<em>Cet email est destiné à '.$salesperson->getCivility().' '.$salesperson->getSurname().' '.$salesperson->getName().' du Fournisseur '.$nameProvider.'.';
                ...
                $altBody  = "Bonjour,\n\n";
                $altBody .= "Vous pouvez dès à présent consulter votre planning pour le salon d\'achats mentionné en objet en vous rendant à l'adresse suivante :\n";
                $altBody .= "http://www.scaouest.info/pf_management/planning/planning_loader_sp.php?pf=".$idPf."&sp=".$idSp."&k=".$key."\n\n";
                $altBody .= "Cet email est destiné à ".$salesperson->getCivility()." ".$salesperson->getSurname()." ".$salesperson->getName()." du Fournisseur ".$nameProvider.".";
                ...
                if($successfulSending) { 
                    ...
                    file_put_contents('./errors/log_emails.txt', '['.date('d-M-Y H:i:s e').'] EMAIL ENVOYÉ : COMMERCIAL '.$salesperson->getSurname(). ' '.$salesperson->getName().' '.$salesperson->getEmail()."\n", FILE_APPEND);
                }
                else {
                    file_put_contents('./errors/log_emails.txt', '['.date('d-M-Y H:i:s e').'] EMAIL NON ENVOYÉ #ERR-03 : COMMERCIAL '.$salesperson->getSurname(). ' '.$salesperson->getName()."\n", FILE_APPEND);
                }
            }
            else {
                file_put_contents('./errors/log_emails.txt', '['.date('d-M-Y H:i:s e').'] EMAIL NON ENVOYÉ #ERR-04 : COMMERCIAL '.$salesperson->getSurname(). ' '.$salesperson->getName()."\n", FILE_APPEND);
            }
        }
    }
}
//print_r($counterSuccesfulSendings);
?>
...
<head>
    ...
    <title>PFManagement | Info Planning 2</title>
    ...
    <div id="wrapper">
        ...
        <div class="navbar-header">
            ...
            <ul class="nav navbar-top-links navbar-right">
                <li>
                    <span class="m-r-sm text-muted welcome-message">Info Planning 2</span>
                </li>
                <li>
            ...
            <div class="row wrapper border-bottom white-bg page-heading">
                <div class="col-lg-9">
                    <h2><i class="fa fa-flag" aria-hidden="true"></i> Informer les Commerciaux que leur Planning est disponible</h2>
                    <ol class="breadcrumb">
                        <li class="active">
                            <strong>Administration/Info Planning 2</strong>
                        </li>
                    </ol>
                </div>
                <div class="col-lg-3">
                        ...
                        <div class="ibox float-e-margins">
                            ...
                            <div class="ibox-content">
                                ...
                                <div class="alert alert-danger">
                                    ...
                                    Valider le formulaire aura pour effet d'envoyer un email d'information aux Commerciaux.&nbsp;
                                    Cet email contient un lien vers leurs plannings respectifs.
                                </div>

                                <form class="m-t" role="form" action="#" method="POST">        
                                    
                                    <?php if(isset($_SESSION['purchasingFairConcerned']) && !empty($_SESSION['purchasingFairConcerned'])) { ?>
                                    <button type="submit" id="submitButtonProviders" name="submitButtonProviders" class="btn block full-width m-t-xl m-b-md hvr-icon-forward">Informer les participants</button>
                                    <?php } else { ?>
